  ...
  <div style="padding: 30px">
    ...
    <el-table :data="actionLogList">
      <el-table-column label="所属类型" width="120" sortable>
          ...
          <span style="margin-left: 10px" v-if="scope.row.logType ==1">Info</span>
          <span style="margin-left: 10px" v-if="scope.row.logType ==2">Debug</span>
          <span style="margin-left: 10px" v-if="scope.row.logType ==3">Error</span>
          <span style="margin-left: 10px" v-if="scope.row.logType ==4">Warning</span>          
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            @click="actionLogDetail(scope.row)"
            type="primary"
            size="small"
            >查看详情</el-button
          >
...
<script>
import { getActionLogList,getActionLogById } from "@/api/action.js";
export default {
  data() {
    return {
      formInline: {
