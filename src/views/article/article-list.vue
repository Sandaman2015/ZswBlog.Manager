  ...
  <div style="padding: 30px">
    <el-form :inline="true" :model="formInline" center>
      <el-form-item label="分类筛选" prop="categoryId">
        <el-select v-model="formInline.categoryId" clearable placeholder="请选择文章所属分类">
          <el-option v-for="item in options" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="文章标题" prop="dimTitle">
        <el-input v-model="formInline.dimTitle" placeholder="搜索文章标题">
    ...
    <el-table v-loading="loading" :data="articleList" element-loading-background="rgba(0, 0, 0, 0.2)"
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="primary" size="small" @click="articleEdit(scope.row)">编辑</el-button>
          <el-button type="danger" size="small" @click="articleRemove(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
